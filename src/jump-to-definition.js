...
function provideDefinition(document, position, token) {
    ...
    if (/\/main$/.test(fileName)) {
        ...
        // 这里我们偷懒只做一个简单的正则匹配
        if (new RegExp(`"^(..|.)\\s${word.replace(/\//g, '\\/')}[\\s\\S]*?\\}`, 'gm').test(json)) {
            let destPath = `${workDir}/lib/${word.replace(/"/g, '')}`;
...
module.exports = function(context) {
    ...
    context.subscriptions.push(vscode.languages.registerDefinitionProvider(['gsate'], {
        provideDefinition
    }));
};